...
See [`tests/2D_reverse_KL.py`](tests/2D_reverse_KL.py) (or the notebook [`tests/2D_reverse_KL.ipynb`](tests/2D_reverse_KL.ipynb)) for an end-to-end run that trains an NSF to push a 2D Gaussian source onto a target specified only by an unnormalized energy, then evaluates the result via importance sampling and ESS. Run from the project root:
