...
| `CNF`     | unbounded $\mathbb R^d$                       | ODE      | ODE                  | trace via Hutchinson | very expressive, training only — `F.inv` and density are ODE solves, not closed form |
...
# 4. Maximum expressivity, training only, F^-1 cost is fine? -> CNF.
flow = CNF(dimension=d, frequency=3)
```
