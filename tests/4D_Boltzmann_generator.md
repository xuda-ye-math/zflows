...
- a small inlined `reverse_KL_annealed` is the per-rung loss (the source-energy term drops out of the gradient automatically), and `zflows.loss.loss_compile(reverse_KL_annealed, u_source, u_target, F, c_k)` wraps it once per rung so every batch after the first runs in a fused CUDA graph;
