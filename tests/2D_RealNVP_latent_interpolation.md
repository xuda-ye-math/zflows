...
- The `forward_KL(y, source=u_source, F=flow.t())` loss works unchanged across NSF, NCSF, CNF, and RealNVP — the same training loop transports between flow classes with a one-line constructor swap.
